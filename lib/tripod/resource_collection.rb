  ...
  class ResourceCollection
    ...
    def to_nt
      time_serialization('nt') do
        if @criteria
          @criteria.serialize(:return_graph => @return_graph, :accept_header => "text/plain")
        elsif @sparql_query_str && @resource_class
          # run the query as a describe.
          @resource_class._raw_describe_select_results(@sparql_query_str, :accept_header => "text/plain")
        else
          # for n-triples we can just concatenate them
          nt = ""
